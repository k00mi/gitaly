...
func (s *server) CreateRepositoryFromURL(ctx context.Context, req *pb.CreateRepositoryFromURLRequest) (*pb.CreateRepositoryFromURLResponse, error) {
	if err := validateCreateRepositoryFromURLRequest(req); err != nil {
		return nil, grpc.Errorf(codes.InvalidArgument, "CreateRepositoryFromURL: %v", err)
	}
	...
	if _, err := os.Stat(repositoryFullPath); !os.IsNotExist(err) {
		return nil, grpc.Errorf(codes.InvalidArgument, "CreateRepositoryFromURL: dest dir exists")
	}
	...
	if err != nil {
		return nil, grpc.Errorf(codes.Internal, "CreateRepositoryFromURL: clone cmd start: %v", err)
	}
	if err := cmd.Wait(); err != nil {
		os.RemoveAll(repositoryFullPath)
		return nil, grpc.Errorf(codes.Internal, "CreateRepositoryFromURL: clone cmd wait: %v", err)
	}
	...
	if _, err := s.CreateRepository(ctx, &pb.CreateRepositoryRequest{Repository: repository}); err != nil {
		return nil, grpc.Errorf(codes.Internal, "CreateRepositoryFromURL: create hooks failed: %v", err)
	}

	if err := removeOriginInRepo(ctx, repository); err != nil {
		return nil, grpc.Errorf(codes.Internal, "CreateRepositoryFromURL: %v", err)
	}
