...
func gitLog(writer git.RefsWriter, repo *pb.Repository, from string, to string) error {
	...
	formatFlag := "--pretty=format:" + strings.Join(git.CommitLogFormatFields, "%x00")
...
func (s *server) CommitsBetween(in *pb.CommitsBetweenRequest, stream pb.CommitService_CommitsBetweenServer) error {
	...
	writer := newCommitsBetweenWriter(stream, s.MaxMsgSize)
