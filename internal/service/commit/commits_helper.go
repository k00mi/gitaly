...
func sendCommits(ctx context.Context, sender chunk.Sender, repo *gitalypb.Repository, revisionRange []string, paths []string, literalPathspecs bool, extraArgs ...git.Option) error {
	cmd, err := log.GitLogCommand(ctx, repo, revisionRange, paths, literalPathspecs, extraArgs...)
	if err != nil {
		return err
	}
